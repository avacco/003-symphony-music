...
/**
 * Componente de contenido del reproductor
 * @param song - cancion actual
 * @param songUrl - url de la cancion
 */
const PlayerContent: React.FC<PlayerContentProps> = ({ song, songUrl }) => {
  ...
  const toggleMute = () => {
    if(volume === 0) setVolume(1); else setVolume(0);
  }
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 h-full">
      ...
      <div className="flex md:hidden col-auto w-full justify-end items-center gap-10">
        <AiFillStepBackward onClick={onPlayPrevious} size={30} className="text-neutral-400 cursor-pointer hover:text-white transition" />
        <div onClick={handlePlay} className="w-10 h-10 flex items-center justify-center rounded-full bg-white p-1 cursor-pointer">
