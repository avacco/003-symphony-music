...
const copyBillingDetailsToCostumer = async (uuid: string, payment_method: Stripe.PaymentMethod) => {
...
const manageSubscriptionStatusChange = async (subscriptionId: string, customerId: string, createAction = false) => {
  ...
  console.log("Subscripcion insertada/actualizada: " + subscription.id + "para " + uuid);

  if(createAction && subscription.default_payment_method && uuid) await copyBillingDetailsToCostumer(uuid, subscription.default_payment_method as Stripe.PaymentMethod);
};
